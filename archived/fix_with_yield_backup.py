...
def fix_collisions(
    ...
    for pass_num in range(1, max_passes + 1):
        ...
        for coll in collisions_sorted:
            ...
            if info_setting in ["all", "no"]:
                if attempt_num <= 5:  # Try static for first 5 attempts
                    for agent_id in agents_to_try:
                        idx = agent_id - 1
                        ...
                        if coll['type'] == 'vertex':
                            ...
                            # For obstacle collisions, block the cell AND expand search around it
                            obs_cell = tuple(map(int, coll['cell']))
                            cells_to_block = [obs_cell]
                            # If collision time > 0, also look at previous position to avoid path leading to obstacle
                            if coll['time'] > 0 and current_trajectories[idx] and coll['time'] < len(current_trajectories[idx]):
                                prev_cell = tuple(map(int, current_trajectories[idx][coll['time'] - 1]))
                                if prev_cell not in cells_to_block:
                                    cells_to_block.append(prev_cell)
                        ...
                        if not new_cells_to_block:
                            if verbose:
                                print(f"    All collision cells already in block history, trying larger rewind")
                            # Try with larger rewind
            ...
            if info_setting in ["all", "only_dyn"]:
                if attempt_num <= 5:  # Try dynamic for first 5 attempts
                    for agent_id in agents_to_try:
                        idx = agent_id - 1
                        other_agents = [aid for aid in coll['agents'] if aid != agent_id]
            ...
            # STRATEGY 4: FORCED YIELD (for very persistent collisions)
            if not fixed and attempt_num > 8 and info_setting == "all":
                # Force ANY agent in collision to yield, even if not at goal
                if verbose:
                    print(f"  → FORCED YIELD (persistent collision, attempt {attempt_num})")
                ...
                for agent_id in agents_to_try:
                    ...
                    if success:
                        ...
                        if coll_resolved or len(new_colls) < len(collisions_sorted):  # Accept if reduces collisions
                            current_plans[idx] = new_plan
                            current_trajectories[idx] = new_traj
                            ...
                            if verbose:
                                print(f"  ✓ FORCED YIELD successful for Agent {agent_id}")
                                print(f"    Reduced collisions from {len(collisions_sorted)} to {len(new_colls)}")
    ...
    # Build unique collisions set
    unique_colls = set()
    all_colls_for_count = analyze_collisions(current_trajectories, agent_goals, pristine_static_grid)
    for c in all_colls_for_count:
    ...
    if verbose:
        ...
        print(f"Unique collisions encountered: {len(unique_colls)}")
        print(f"Timed out: {timed_out}")
        info_tracker.report()

    # Build log data (simplified version compatible with original)
    log_data = {
